package com.vagoscorp.vccompostrc;
...
//	Comunic comunic;
	...
	int[] rutX = { 0, 1000, 1000, 0, 0, 1000, 1000, 0, 0, 1000, 1000, 0, 0 };
	int[] rutZ = { 0, 0, 0, 0, 333, 333, 666, 666, 1000, 1000, 1000, 1000, 0 };
	int[] rutW = { 0, 0, 60, 60, 60, 60, 70, 70, 80, 80, 90, 90, 0 };
	Grua gru;
	EjeZ ejeZ;
	boolean run = false;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (BTAdapter == null) {
			Toast.makeText(this, R.string.NB, Toast.LENGTH_SHORT)
					.show();
			finish();
			return;
		}
...
//		serverport = shapre.getInt(SP, defPort);
	...
	public void onBackPressed() {
		// Toast.makeText(this, "Cierre la APP desde el men�",
...
//			comunic = new Comunic(this, serverip, serverport);
			...
			comunic.setConnectionListener(new OnConnectionListener() {
				...
				public void onConnectionstablished() {
					Log.d("Client", "Conexi�n Establecida");

					comunic.enviar("X=" + Float.toString(xval) + "=Y="
							+ Float.toString(yval) + "=Z="
							+ Float.toString(zval) + "=A="
							+ Float.toString(wval) + "=/");
					fCon = true;
					datoRCV = true;
					// timeOut_init();
				...
				public void onConnectionfinished() {
					Log.d("Client", "Conexi�n Finalizada");
					if (fCon) {
						// timeOut_end();
						fCon = false;
					}
	...
	public void iniciar_Cliente() {
		...
		final float zval = (float) (ejeZ.yTouch
				- (float) Math.sin(Math.toRadians(wval)) * ejeZ.lw - ejeZ.minY)
				/ ejeZ.ez;
		// float adyacente = ejeZ.xTouch - ejeZ.cent;
		// float ang = (float)Math.acos(adyacente / ejeZ.lw);
		// float opuesto = (float)Math.sin(ang)*ejeZ.lw;
...
//							+ Float.toString(wval) + "=/");
					comunic.enviar('/');
					comunic.enviar(Math.round(xval/4));
					comunic.enviar(Math.round(yval/4));
					comunic.enviar(Math.round(wval/4));
					comunic.enviar(';');
					fCon = true;
					datoRCV = true;
					// timeOut_init();
	...
	@Override
	public void onDataReceived(String dato, int[] ndato) {// IL Dato Recibido
		tarea += dato;
		if (dato.endsWith("/")) {
			// timeOut_end();
			datoRCV = true;
			...
			onPause();
			gru.xActual = (float) (gru.ex * Float.parseFloat(vals[1]))
					+ gru.limX;
			gru.yActual = (float) (gru.ey * Float.parseFloat(vals[3]))
					+ gru.limY;
			float wActual = (float) Math.toRadians(Float.parseFloat(vals[7]));
			ejeZ.yActual = (float) (ejeZ.ez * Float.parseFloat(vals[5]))
					+ ejeZ.minY + (float) Math.sin(wActual) * ejeZ.lw;
	...
	// gru.canvas.drawLine(gru.Width - wPat, 0, gru.Width - wPat, gru.Height,
	// gruPaint);
	// gru.canvas.drawLine(gru.Width - mwPat, 0, gru.Width - mwPat, gru.Height,
	// gruPaint);
	// gru.canvas.drawLine(gru.Width - 1, 0, gru.Width - 1, gru.Height,
	// gruPaint);
	// gru.canvas.drawRect(0, y - hPat, wPat, y + hPat, grua);
	// gru.canvas.drawRect(gru.Width - wPat, y - hPat, gru.Width, y + hPat,
	// grua);
	...
	// gru.canvas.drawLine(0, gru.Height - hPat, gru.Width, gru.Height - hPat,
	// gruPaint);
	// gru.canvas.drawLine(0, gru.Height - mhPat, gru.Width, gru.Height - mhPat,
	// gruPaint);
	// gru.canvas.drawLine(0, gru.Height - 1, gru.Width, gru.Height - 1,
	// gruPaint);
	// gru.canvas.drawRect(x - wPat, 0, x + wPat, hPat, grua);
	// gru.canvas.drawRect(x - wPat, gru.Height - hPat, x + wPat, gru.Height,
	// grua);
	...
	// ejeZ.canvas.drawRect(0, ejeZ.Height - wPat, ejeZ.Width, ejeZ.Height,
	// grua);
	// ejeZ.canvas.drawRect(ejeZ.Width/2 - wPat, 0, ejeZ.Width/2 + wPat,
	// ejeZ.Height, grua);
	// ejeZ.canvas.drawRect(ejeZ.Width/2 - wCar, 0, ejeZ.Width/2 + wCar, hPat,
	// carr);
	// ejeZ.canvas.drawRect(ejeZ.Width/2 - ejeZ.dx*30, hPat, ejeZ.Width/2 +
	// ejeZ.dx*30, hPat + hPat, carr);
	// }
